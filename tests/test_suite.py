...
class ParametersSetup:
    ...
    def populate_file_list(self):
        images_files_path = str(Path(__file__).parent.joinpath("example/test_images/").resolve())
        self.file_list = []
    ...
    def populate_params(self):
        for file_path in self.file_list:
            ...
            self.is_valid_params.append(("factor_object", file_path,
                                         eval("dataResult.{}().is_valid_result".format(
                                             posix_path.suffix[1:].capitalize() + posix_path.stem))))
            self.estimate_age_params.append(("factor_object", file_path,
                                             eval("dataResult.{}().estimate_age_result".format(
                                                 posix_path.suffix[1:].capitalize() + posix_path.stem))))
            self.get_iso_params.append(("factor_object", file_path,
                                        eval("dataResult.{}().get_iso_result".format(
                                            posix_path.suffix[1:].capitalize() + posix_path.stem))))
            self.compare_params.append(("factor_object", file_path,
                                        eval("dataResult.{}().compare_result".format(
                                            posix_path.suffix[1:].capitalize() + posix_path.stem))))
            self.epd_params.append(("factor_object", file_path,
                                    eval("dataResult.{}().enroll_result".format(
                                        posix_path.suffix[1:].capitalize() + posix_path.stem)),
                                    eval("dataResult.{}().predict_result".format(
                                        posix_path.suffix[1:].capitalize() + posix_path.stem)),
                                    eval("dataResult.{}().delete_result".format(
                                        posix_path.suffix[1:].capitalize() + posix_path.stem))
                                    ))
...
@pytest.mark.parametrize("factor_object, file_name, expected", ParametersSetup().is_valid_params,
...
def test_valid(factor_object, file_name, expected):
    ...
    for expected, actual in zip(sorted(expected.face_objects, key=lambda x: x.bounding_box.top_left_coordinate.x),
        ...
        assert (actual.bounding_box.top_left_coordinate.__str__() ==
                expected.bounding_box.top_left_coordinate.__str__())
        assert (actual.bounding_box.bottom_right_coordinate.__str__() ==
                expected.bounding_box.bottom_right_coordinate.__str__())


@pytest.mark.parametrize("factor_object, file_name, expected", ParametersSetup().estimate_age_params,
                         indirect=["factor_object"])
...
@pytest.mark.parametrize("factor_object, file_name, expected", ParametersSetup().get_iso_params,
                         indirect=["factor_object"])
def test_get_iso(factor_object, file_name, expected):
    ...
    if actual.confidence is None:
        ...
        assert (expected.confidence - 0.5 <= actual.confidence <= expected.confidence + 0.5)


@pytest.mark.parametrize("factor_object, file_name, expected", ParametersSetup().compare_params,
                         indirect=["factor_object"])
...
@pytest.mark.parametrize("factor_object, file_name, enroll_expected, predict_expected, delete_expected",
                         ParametersSetup().epd_params,
                         indirect=["factor_object"])
def test_epd(factor_object, file_name, enroll_expected, predict_expected, delete_expected):
