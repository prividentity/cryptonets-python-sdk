...
- Fixed use-after-free bug by copying buffer to bytes before freeing C-allocated memory as `ffi.buffer()` doest not copy the internal CData buffer
- Fixed Native SDK version string initialization (was returned empty)
